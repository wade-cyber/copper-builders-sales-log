...
import {
  ...
  LEADS_SHEET_ID, RESULTS_SHEET_ID,
} from './_lib/sheets.js';

export default async function handler(req, res) {
  try {
    // Copper Leads Dashboard — community names from rows 3+
    const leadsNames = [];
    try {
      const ld = await getSheetData(LEADS_SHEET_ID, 'Dashboard!A3:B');
    ...
    // Sales Data Results — community names from all rows
    const resultsNames = [];
    const resultsTabNames = [];

    const meta = await getSpreadsheet(RESULTS_SHEET_ID);
    for (const sheet of meta.sheets) {
      resultsTabNames.push(sheet.properties.title);
    }

    try {
      const rd = await getSheetData(RESULTS_SHEET_ID, 'Sales Data Results!A:B');
